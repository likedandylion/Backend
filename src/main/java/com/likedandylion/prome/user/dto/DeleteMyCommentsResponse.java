 ...
 * - deletedIds: 실제로 삭제된 댓글 ID 목록
 * - skippedIds: 요청했지만 삭제되지 않은 ID들(존재X/내 소유 아님 등)
 * - deletedCount: 삭제된 개수
 */
public record DeleteMyCommentsResponse(
        List<Long> deletedIds,
        List<Long> skippedIds,
        int deletedCount
) {
    /**
     * 편의 생성자: deletedIds/ skippedIds만 주면 deletedCount는 자동 계산.
     */
    public static DeleteMyCommentsResponse of(List<Long> deletedIds, List<Long> skippedIds) {
